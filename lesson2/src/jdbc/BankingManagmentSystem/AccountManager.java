...
public class AccountManager {
    ...
    public void debitMoney(long account_number) throws SQLException {
        ...
        try {
            ...
            if (account_number != 0) {
                PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM account WHERE account_number = ? AND seccurity_pin = ?");
    ...
    public void getBalance(long account_number){
        ...
        String security_pin = input.nextLine();
        try{
            PreparedStatement preparedStatement = connection.prepareStatement("SELECT balance FROM account WHERE account_number = ? AND seccurity_pin = ?");
            preparedStatement.setLong(1, account_number);
            preparedStatement.setString(2, security_pin);
            ResultSet resultSet = preparedStatement.executeQuery();
            if (resultSet.next()){
